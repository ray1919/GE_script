...
ComBat <- function(expression_xls='data.txt', sample_info_file='batch.txt', type='txt', write=T, covariates='all', par.prior=T, filter=F, skip=1, prior.plots=F){
  ...
  if(write){
    output_file <- c('Combated_data.txt')
                 #print(geneinfo[1:2])
    ...
    write.table(outdata, file=output_file, sep="\t",row.names=F,quote=F)
    cat("Adjusted data saved in file:",output_file,"\n")
    if ( file.exists("anns.txt") & file.exists("flag.txt") ) {
      system('cut -f2-30 anns.txt > a.tmp')
      system('paste Combated_data.txt flag.txt a.tmp > Combated_all.txt')
      unlink('a.tmp')
    }
