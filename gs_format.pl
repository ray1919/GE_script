...
$thread = async {
    ...
    for ( $i = 0 ; $i < @sDiff ; $i++ ) {
        ...
        foreach $reg ('up','down') {
          ...
          @title = $regTbl->header;
          @title = map { $_ =~ s/raw/normalized/; $_ } $regTbl->header if $plat == 1;
          $sheet->merge_range(2, 0, 2, $regTbl->nofCol - 1,
            "$sc $fc[$i] fold $reg regulated genes",
            $reg eq 'up' ? $UP : $DOWN
          );
          &title_format;
...
sub title_format {
    ...
    if ( !defined $pc[$i] ) {
      if ( $plat == 2 ) {
        ...
        $sheet->merge_range( 3, 1, 3, $title_id{'Regulation'}, 'Fold change and Regulation', $ANN );
        $sheet->merge_range( 3, $title_id{'group_raw'}, 3, $title_id{'ann'} - 1, 'Normalized Intensity', $RI );
        $sheet->merge_range( 3, $title_id{'ann'}, 3, $title_id{'end'}, 'Annotation', $ANN );
      }
    }
    else {
      if ( $plat == 2 ) {
        ...
        $sheet->merge_range( 3, 1, 3, $title_id{'Regulation'}, defined $pc[$i] ?
          'P-value Fold change and Regulation'
          : 'Fold change and Regulation', $ANN );
        $sheet->merge_range(3, $title_id{'group_raw'},3,$title_id{'spl_raw'} - 1, 'Group-Normalized Intensity', $RI );
        $sheet->merge_range(3,$title_id{'spl_raw'},3,$title_id{'ann'} - 1,'Normalized Intensity', $GNI );
        $sheet->merge_range(3,$title_id{'ann'},3, $title_id{'end'},'Annotation', $ANN );
      }
    }
    $sheet->set_column( 0, $regTbl->nofCol - 1, 10 );
...
sub set_note2 {
    ...
    my %title_id;
    foreach $id ('p-value','FCAbsolute','Regulation','Absolute Fold change','Log Fold change',) {
      $k = 1;
      @idx = ();
    ...
    if ( $gn > 0 ) {
      if ( $plat == 2 ) {
        ...
        $title_id{'group_nor'} = &char($title_id{'group_raw'} + 1) . '~' . &char($title_id{'spl_raw'});
        $title_id{'spl_nor'} = &char($title_id{'spl_raw'} + 1) . '~' . &char($ann_idx);
      }
    }
    else {
      if ( $plat == 2 ) {
        ...
        $title_id{'spl_nor'} = &char($title_id{'group_raw'} + 1) . '~' . &char($ann_idx);
      }
    }
    ...
    if ( !defined $pc[$i] ) {
        $ann_ids = join(', ',@title[$ann_idx .. ($regTbl->nofCol - $plat)]);
        $noteUD = $plat == 2
          ? "# Condition pairs: $sc
...
# Column A: SEQ_ID, the sequence identifier.
# Column $title_id{'Absolute Fold change'}: Absolute Fold change, the absolute ratio (no log scale) of normalized intensities between two samples.
# Column $title_id{'Regulation'}: Regulation, it depicts which sample has greater or lower intensity values wrt other sample.
# Column $title_id{'spl_nor'}: Normalized Intensity of each sample, the gene expression summary value for the gene.
# Column $title_id{'ann'}: Annotations for each gene, including $ann_ids.";
    }
    else {
        $ann_ids = join(', ',@title[$ann_idx .. ($regTbl->nofCol - $plat)]);
