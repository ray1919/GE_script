...
sub do_subgroup {
  ...
  foreach $sgFile ('Enhancer_LncRNA_profiling','HOX_cluster_profiling','Rinn_lincRNA_profiling') {
    next unless (-e "$tmpdir/$sgFile.txt");
    ### ---------- prepare : $sgFile.' ----------------'
...
sub title_format {
    if ( $gn == 0 || !defined $pc[$i]) {
        ...
        $sheet->merge_range( 3, 1, 3, 3, defined $pc[$i] ?
...
sub set_note2 {
    ...
    if ( !defined $pc[$i] && $snp == 2 ) {
        $ann_ids = join(', ',@title[ $sExp_ann_idx .. ($sExp->nofCol - 1)]);
...
# Column A: ProbeName, it represents probe name.
# Column B: Log Fold change, the log2 value of absolute ratio of normalized intensities between two groups.
# Column C: Absolute Fold change, the absolute ratio (no log scale) of normalized intensities between two groups.
...
# Column " . char( 9 + 2 * $snp ) . "~" . char( $regTbl->nofCol )
    ...
    if ($wd eq 'LncRNA') {
      ...
      $noteUD .= '
Note:
# Column '.char(12+2*$snp).': source, the source of LncRNA.
RefSeq_NR: RefSeq validated non-coding RNA;
UCSC_knowngene: UCSC known genes annotated as "non-coding", "near-coding" and "antisense" (http://genome.ucsc.edu/cgi-bin/hgTables/);
Ensembl: Ensembl (http://www.ensembl.org/index.html);
H-invDB: H-invDB (http://www.h-invitational.jp/);
